...
function deletRowArr(k){
    ...
    for(var i=k;i<9;i++){
        ...
        do{//生成行，并检测3*3盒子和列
            ...
            if(i>k&&i<8&&k>=3){//note:1：i=k时的值已不重复，不需要检测.2：i!=8,即不能自己检测是否和自己相等.
            ...
            count++;
            if(count>10){//循环次数超过10次时，本轮生成的已有的行都归零，在重新生成
                count = 0;
...
function layout(){
    ...
    while(k<9){//生成行、列
        do{//生成行
            ...
            roNum++;
            if(roNum>10||temp.length<1){//超过循环次数或temp为空（即temp内的数字均与现有的行列或3*3盒子内的数字重复时，temp内的数字均被排除）
